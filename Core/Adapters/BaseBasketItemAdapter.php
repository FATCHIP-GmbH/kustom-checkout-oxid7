...
//        }
    ...
    /**
     * Compares Kustom Order Line to oxid basket object
     * @param $orderLine
     */
     ...
     * @param $updateData array - Update data to send to Kustom
     * @return void
     */
    ...
    /**
     * Calculate tax on Kustom values
     * @param int $total
     * @param int $rate
     * @return float
     */
    ...
    protected function getKustomType() {
    ...
    protected function validateData($orderLine, $key, $basketValue) {
        ...
        KustomUtils::log('debug', 'VALIDATING: ' . print_r([
            'type' => $this->getType(),
            'key' => $key,
        ...
        if ($orderLine[$key] !== $basketValue) {
            $oEx = new InvalidItemException("FCKUSTOM_INVALID_ITEM");
            $this->setDiffData([
                'key' => $key,
                'requestedValue' => $orderLine[$key],
                'basketValue' => $basketValue
