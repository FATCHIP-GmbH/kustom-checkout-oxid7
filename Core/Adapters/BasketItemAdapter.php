...
class BasketItemAdapter extends BaseBasketItemAdapter
{
    ...
    public function handleUpdate(&$updateData)
    {
        ...
        if ($this->diffData['key'] === 'quantity') {
            ...
            try {
                ...
                KustomUtils::logException($changeBasketException);
            }
        }
        ...
        KustomUtils::log('debug', 'ITEM_UPDATED: ' .
            print_r([
    ...
    public function prepareItemData($iLang)
    {
        ...
        $name = $oArticle->getFieldData('OXTITLE') . ' BasketItemAdapter.php' . $oArticle->getFieldData('OXVARSELECT');
        ...
        $this->itemData['product_url'] = $oArticle->fcKustom_getArticleUrl();
        $this->itemData['image_url'] = $oArticle->fcKustom_getArticleImageUrl();
        $this->itemData['product_identifiers'] = array(
            'category_path'            => $oArticle->fcKustom_getArticleCategoryPath(),
            'global_trade_item_number' => $oArticle->fcKustom_getArticleEAN(),
            'manufacturer_part_number' => $oArticle->fcKustom_getArticleMPN(),
            'brand'                    => $oArticle->fcKustom_getArticleManufacturer(),
        );
    ...
    /**
     * Compares Kustom OrderData price to oItem  price representing oxid basket item
     * @param $orderLine
