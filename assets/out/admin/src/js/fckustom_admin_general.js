...
function FormRows(accSelector) {
    ...
    this.rowTemplate = fckustom_countryCredsTemplate;
    this.newItem = {};
...
FormRows.prototype.init = function () {
    ...
    $('.kustom-conf-table').delegate('.acc-remove', 'click', this.removeItem.bind(this));
};
...
FormRows.prototype.setAttrs = function (element, countryISO) {
    ...
    switch (element.nodeName) {
        ...
        case 'INPUT':
            element.name = element.name.replace('aKustomCreds_', 'aKustomCreds_' + countryISO);
            element.setAttribute('value', this.newItem.values.shift());
            break;

        case 'DIV':
            if (element.classList.contains('kustom-flag'))
...
FormRows.prototype.createItem = function () {
    ...
    $template.get(0).className = 'kustom-creds_' + this.getCountryISO() + ' csc_first';
    $template.find('div.kustom-flag, div.rows-wrapper, input, a.acc-remove').each(
        (function (i, element) {
    ...
    setTimeout(function () {
        $template.find('.rows-wrapper').toggle(400);
    }, 10);
    // adds css animation
};
...
var modeSelector = new Selector2({
    ...
    kcomethodSelector = new Selector2({
                                        id: 'kcomethodSelector',
                                        fromOptions: false
                                    }),
    accSelector = new CountrySpecificCredentialsSelector({
                                                             id: 'accSelector',
                                                             fromOptions: false,
                                                             options: fckustom_countriesList
                                                         }),
