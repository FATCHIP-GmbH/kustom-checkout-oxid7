...
/**
 * Class for Kustom payment history action handling
 *
 * @package Kustom
 */
class KustomPaymentHistoryFull
{
    /**
    ...
    protected function getPaymentStatistics(Payment $payment, $userId)
    {
        if (!isset($this->paymentStatistics[$payment->getId()])) {
            ...
            $orderTable = $oTableViewNameGenerator->getViewName('oxorder');

            [$queryConditions, $params] = $this->getPaymentQueryConditions($payment, $userId);
            $query = "
              SELECT
    ...
    protected function getPaymentQueryConditions(Payment $payment, $userId)
    {
        ...
        if (($st = $this->getSuccessfulOrderStatuses()) && is_array($st)) {
            $whereCondition .= " AND {$orderTable}.OXTRANSSTATUS IN (:transStatus)";
            $params[':transStatus'] = $st;
        }
    ...
    {
        return $payment->oxpayments__fckustom_emdpurchasehistoryfull->value == KustomConsts::EMD_ORDER_HISTORY_PAID;
    }
    ...
    {
        return $payment->oxpayments__fckustom_emdpurchasehistoryfull->value != KustomConsts::EMD_ORDER_HISTORY_NONE;
    }
    ...
    protected function addPaymentStatisticsToHistory($historyRecords, $payment, $userId)
    {
        if ($this->isFullPaymentHistoryPossible($payment, $userId)) {
            $paymentType       = $payment->oxpayments__fckustom_paymentoption->value;
            ...
            if (!isset($historyRecords[$paymentType])) {
                $historyRecords[$paymentType] = array(
                    ...
                    "payment_option"              => $payment->oxpayments__fckustom_paymentoption->value,
    ...
    protected function modifyDateFormats($historyRecords)
    {
        foreach ($historyRecords as &$statistics) {
            ...
            $statistics["date_of_last_paid_purchase"] = $dateLastPaid->format(KustomEMD::EMD_FORMAT);
            ...
            $statistics["date_of_first_paid_purchase"] = $dateFirstPaid->format(KustomEMD::EMD_FORMAT);
        }
