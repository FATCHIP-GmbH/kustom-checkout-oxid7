...
use Fatchip\FcKustom\Controller\Admin\KustomOrderArticle;
...
class KustomOrderArticleTest extends ModuleUnitTestCase {
    protected function setOrder($oxstorno = 0) {
        ...
        $mockBuilder->setMethods(['load', 'save', 'getTotalOrderSum', 'getNewOrderLinesAndTotals', 'updateKustomOrder']);
    ...
    public function testInit() {
        ...
        $controller = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['authorize', 'getEditObjectId', 'isCredentialsValid'])->getMock();
        ...
        $controller = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['authorize', 'getEditObjectId', 'isCredentialsValid', 'retrieveKustomOrder'])->getMock();
        $controller->expects($this->once())->method('authorize')->willReturn(true);
        $controller->expects($this->any())->method('getEditObjectId')->willReturn('test');
    ...
    public function testInitExceptions($exception, $expected) {
        $this->setOrder();
        $controller = $this->getMockBuilder(KustomOrderArticle::class)
    ...
    public function testRender() {
        $order = $this->setOrder();
        $orderMain = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['authorize'])->getMock();
        ...
        $orderMain = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['isKustomOrder', 'isCredentialsValid'])->getMock();
        $orderMain->expects($this->any())->method('isKustomOrder')->willReturn(true);
        ...
        $orderMain = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['authorize', 'isCredentialsValid', 'isCaptureInSync'])->getMock();
        $orderMain->expects($this->any())->method('authorize')->willReturn(true);
        $orderMain->expects($this->once())->method('isCredentialsValid')->willReturn(true);
    ...
    public function testIsCaptureInSync($kustomOrderData, $expected, $withOrder = false) {
        ...
        $controller = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['getEditObjectId'])->getMock();
        $controller->expects($this->any())->method('getEditObjectId')->willReturn('test');
        if ($withOrder) {
            $order = $this->setOrder();
    ...
    public function testStorno() {
        ...
        $controller = $this->getMockBuilder(KustomOrderArticle::class)->setMethods(['getEditObjectId', 'isKustomOrder'])->getMock();
        $controller->expects($this->any())->method('getEditObjectId')->willReturn('test');
        $controller->expects($this->once())->method('isKustomOrder')->willReturn(true);
        $controller->storno();
        $result = $controller->getViewData()['sErrorMessage'];
